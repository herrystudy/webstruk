...
class DatatableExtend {
    ...
    _addListeners() {
        ...
        // Colvis listeners
        document.querySelectorAll(".datatable-colvis").forEach((el) => {
            el.addEventListener("click", this._onColvisClick.bind(this));
        });

        // Copy listeners
        document.querySelectorAll(".datatable-copy").forEach((el) => {
            el.addEventListener("click", this._onCopyClick.bind(this));
        });
    ...
    _onColvisClick(event) {
        event.preventDefault();
        const index = event.target.getAttribute("data-column");

        // Get the column API object
        var column = this._getDatatable(event.currentTarget).column(index);
    ...
    _onCopyClick(event) {
        event.preventDefault();
        try {
            this._getDatatable(event.currentTarget).buttons(0).trigger();
        } catch (error) {
            console.log("Trigger button is not found");
        }
    }

/**
 *
 * BoxedVariations
 *
 * Interface.Plugins.Datatables.BoxedVariations page content scripts. Initialized from scripts.js file.
 *
 *
 */

class BoxedVariations {
    constructor() {
        if (!jQuery().DataTable) {
            console.log("DataTable is null!");
            return;
        }

        this._dataTableScroll = null;

        this._initBoxedWithScrollbar();
        this._initBoxedWithStandard();
        this._initBoxedWithHidden();
        this._initBoxedWithPrices();
        this._initBoxedWithDownline();
        this._initBoxedWithStandardResponsive();
        this._initBoxedWithHiddenResponsive();
        this._initBoxedWithDownlineResponsive();
        this._initBoxedWithPricesResponsive();
        this._extendDatatables();
    }

    // Boxed variation with scrollbar
    _initBoxedWithScrollbar() {
        const _this = this;
        jQuery(".data-table-scrollable").DataTable({
            destroy: true,
            paging: false,
            buttons: ["copy", "excel", "csv", "print", "pdf", "colvis"],
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            responsive: true,
            scrollY: "100%",
            scrollCollapse: !0,
            fnInitComplete: function () {
                _this._dataTableScroll = OverlayScrollbars(
                    document.querySelectorAll(".dataTables_scrollBody"),
                    {
                        scrollbars: { autoHideDelay: 300 },
                        overflowBehavior: { x: "hidden", y: "scroll" },
                    }
                );
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithStandard() {
        jQuery(".data-table-standard").DataTable({
            destroy: true,
            paging: true,
            buttons: ["copy", "excel", "csv", "print", "colvis"],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            scrollX: true,
            // responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithHidden() {
        jQuery(".data-table-hidden").DataTable({
            destroy: true,
            paging: true,
            buttons: [
                {
                    extend: "copy",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                ,
                {
                    extend: "excel",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "csv",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "print",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                "pdf",
                "colvis",
            ],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    visible: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            scrollX: true,
            // responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithDownline() {
        jQuery(".data-table-downline").DataTable({
            destroy: true,
            paging: true,
            buttons: [
                {
                    extend: "copy",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                ,
                {
                    extend: "excel",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "csv",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "print",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                "pdf",
                "colvis",
            ],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    visible: false,
                    searchable: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
                {
                    targets: 1,
                    visible: false,
                    searchable: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            scrollX: true,
            // responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithPrices() {
        jQuery(".data-table-prices").DataTable({
            destroy: true,
            paging: false,
            ordering: false,

            length: 100,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    // visible: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            scrollX: true,
            // responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithStandardResponsive() {
        jQuery(".data-table-standardresponsive").DataTable({
            destroy: true,
            paging: true,
            buttons: ["copy", "excel", "csv", "print", "colvis"],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            // scrollX: true,
            responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithHiddenResponsive() {
        jQuery(".data-table-hiddenresponsive").DataTable({
            destroy: true,
            paging: true,
            buttons: [
                {
                    extend: "copy",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                ,
                {
                    extend: "excel",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "csv",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "print",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                "pdf",
                "colvis",
            ],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    visible: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            // scrollX: true,
            responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithDownlineResponsive() {
        jQuery(".data-table-downlineresponsive").DataTable({
            destroy: true,
            paging: true,
            buttons: [
                {
                    extend: "copy",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                ,
                {
                    extend: "excel",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "csv",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                {
                    extend: "print",
                    exportOptions: {
                        columns: ":visible",
                    },
                },
                "pdf",
                "colvis",
            ],
            length: 10,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    visible: false,
                    searchable: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
                {
                    targets: 1,
                    visible: false,
                    searchable: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            // scrollX: true,
            responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Boxed variation for pagination, hover and stripe examples
    _initBoxedWithPricesResponsive() {
        jQuery(".data-table-pricesresponsive").DataTable({
            destroy: true,
            paging: false,
            ordering: false,

            length: 100,
            columnDefs: [
                // Adding Name content as an anchor with a target #
                {
                    targets: 0,
                    // visible: false,

                    render: function (data, type, row, meta) {
                        return (
                            '<a class="list-item-heading body" href="#">' +
                            data +
                            "</a>"
                        );
                    },
                },
            ],
            sDom: '<"row"<"col-sm-12"<"table-container"t>ir>><"row"<"col-12"p>>',
            // scrollX: true,
            responsive: true,
            language: {
                paginate: {
                    previous: '<i class="cs-chevron-left"></i>',
                    next: '<i class="cs-chevron-right"></i>',
                },
            },
        });
    }

    // Calling extend makes search, page length, print and export work
    _extendDatatables() {
        new DatatableExtend();
    }
}
